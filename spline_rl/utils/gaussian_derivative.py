...
class dGaussianRBF:
    r"""
    Class implementing the derivative of Gaussian radial basis functions. The value of the feature is computed using the formula:
    ...
    def __call__(self, x):
        ...
        return -2 * (x - self._mean) / self._scale * np.exp(-np.sum((x - self._mean)**2 / self._scale))
    ...
    def generate(n_centers, low, high, dimensions=None, eta=0.25):
        ...
        for i in range(len(grid)):
            v = grid[i, :]
            bf = dGaussianRBF(v, b, dimensions)
            basis.append(bf)
...
class ddGaussianRBF:
    ...
    def __call__(self, x):
        ...
        return (4 * (x - self._mean)**2 + 2 * self._scale) / self._scale**2 * np.exp(-np.sum((x - self._mean)**2 / self._scale))
    ...
    def generate(n_centers, low, high, dimensions=None, eta=0.25):
        ...
        for i in range(len(grid)):
            v = grid[i, :]
            bf = ddGaussianRBF(v, b, dimensions)
            basis.append(bf)
