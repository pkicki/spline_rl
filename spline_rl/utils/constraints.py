...
class BoxPushingConstraints(Constraint):
    def __init__(self, q_min, q_max, q_dot_max, q_ddot_max) -> None:
        self.q_min = q_min
        self.q_max = q_max
        self.q_dot_max = q_dot_max
        self.q_ddot_max = q_ddot_max
        ...
        self.robot = DifferentiableRobotModel(urdf_path=self.urdf_path, name="panda")
    ...
    def evaluate(self, q, q_dot, q_ddot, dt):
        ...
        dt_ = dt[..., None]
        # Prepare the constraint limits tensors
        q_lb = torch.Tensor(self.q_min)[None, None]
        q_ub = torch.Tensor(self.q_max)[None, None]
        q_dot_limits = torch.Tensor(self.q_dot_max)[None, None]
        q_ddot_limits = torch.Tensor(self.q_ddot_max)[None, None]

        q_ub_loss = limit_loss(q, dt_, q_ub)
        q_lb_loss = limit_loss(-q, dt_, -q_lb)
        q_loss = torch.max(q_ub_loss, q_lb_loss)
        q_dot_loss = limit_loss(torch.abs(q_dot), dt_, q_dot_limits)
        q_ddot_loss = limit_loss(torch.abs(q_ddot), dt_, q_ddot_limits)
