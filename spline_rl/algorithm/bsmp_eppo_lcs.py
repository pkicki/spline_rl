...
class BSMPePPOLCS(ePPO):
    ...
    def __init__(self, mdp_info, distribution, policy, optimizer, value_function, value_function_optimizer,
                 constraint_lr, constraints_scale, constraints_scale_optimizer,
                 n_epochs_policy, batch_size, eps_ppo, ent_coeff=0., context_builder=None): 
        ...
        self.constraints_scale_optimizer = constraints_scale_optimizer
    ...
    def update_constraint_scaling(self, constraint_losses, constraints_scale):
        self.constraints_scale_optimizer.zero_grad()
        constraint_losses = constraint_losses.detach()
        alphas_update = self.constraint_lr * torch.log(
            (constraint_losses + self.constraints.violation_limits * 1e-1) / self.constraints.violation_limits)
        desired_constraints_scale = constraints_scale.detach() + alphas_update
        desired_constraints_scale = torch.clip(desired_constraints_scale, -7., 37.)

        constraints_scale_loss = torch.mean((constraints_scale - desired_constraints_scale)**2)
        constraints_scale_loss.backward()
        self.constraints_scale_optimizer.step()


    def get_alphas(self, context):
        return torch.mean(self.constraints_scale(context), axis=0)
    ...
    def _update(self, Jep, theta, context):
        ...
        for epoch in range(self._n_epochs_policy()):
            for minibatch in minibatch_generator(self._batch_size(), *full_batch):
                ...
                constraints_scale = self.constraints_scale(context_i)
                constraint_loss = torch.exp(constraints_scale.detach()) * constraint_losses
                ...
                self.update_constraint_scaling(constraint_losses, constraints_scale)
            self._epoch_no += 1