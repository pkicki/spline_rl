...
class KinodynamicCupAccEnv(AccelerationControl, MuJoCo):
    def __init__(self, gamma=0.99, horizon=150, return_cost=True, timestep=1 / 1000., n_intermediate_steps=20, n_substeps=1,
                 viewer_params={}, **kwargs):
        ...
        self.info.action_space = Box(low=self.env_info['robot']['joint_acc_limit'][0],
                                     high=self.env_info['robot']['joint_acc_limit'][1])
    ...
    def _create_info_dictionary(self, state):
        ...
        task_info['cost'] = np.stack([task_info['joint_pos_constraint'], task_info['joint_vel_constraint'],
                                      task_info['orientation_constraint'], task_info['collision_constriant']])
    ...
    def step(self, action):
        obs, reward, done, info = super(KinodynamicCupAccEnv, self).step(action)
